...
void forward_left(void)
{
	angle.left=distance.left-last_distance.left;
	/*------------------根据 angle.left 进行控制--------------*/
	printf("forward left:%d\t%d\t%d\r\n",distance.left,distance.right,distance.front);
	if(angle.left>0)
	{
		printf("face to right,need to left()\r\n");
		left();
		
	}
	else if(angle.left<0)
	{
		printf("face to left,need to right()\r\n");
		right();
	}
	...
	{
		printf("too left,need to right()\r\n");
		right();
	}
	else if(distance.left-LEFT_DIS>THRESHOLD)
	{
	  printf("too right ,need to left();\r\n");
		left();
		delay_ms(200);
	}
	else
...
void turn_left(void)
{
	printf("turn  left:%d\t%d\t%d\r\n",distance.front,distance.left,distance.right);
  MotorRight(TIM4,700);
  MotorLeft(TIM4,700);
	delay_ms(620);
	stop();
