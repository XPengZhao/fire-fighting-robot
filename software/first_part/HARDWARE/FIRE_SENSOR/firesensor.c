...
{
	GPIO_InitTypeDef GPIO_InitStructure;
	RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOC, ENABLE); //使能PA,PD 端口时钟
...
void Get_Fire(void)
   ...
   sig.left= GetVolt(After_filter[0]);
	 ...
	 sig.mid=GetVolt(After_filter[2]);
		//printf("sig.left:%d   sig.right:%d  \r\n",sig.left,sig.right);
}
...
{ int i=0;
	...
	stop();
	delay_ms(1000);
	while((sig.mid<20||sig.left<20||sig.right<20)&&i<4)
		{
		delay_ms(500);
			Get_Fire();
			i++;
...
/*void PutOff_RightFire(void)
{
	...
	while(sig.right<50&&sig.left<50)
	{
		MotorRight(TIM4,800);
		MotorLeft(TIM4,800);
		delay_ms(100);
...
void PutOff_LeftFire(void)
{
	int n=0;
	Get_Fire();
	while(sig.left<50&&sig.right<50)
	{
		MotorRight(TIM4,700);
		MotorLeft(TIM4,700);
		delay_ms(100);
