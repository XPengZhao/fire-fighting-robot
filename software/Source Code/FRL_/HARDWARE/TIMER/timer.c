...
{	 
	...
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IPU; 		    //PA1 上拉输入
	GPIO_Init(GPIOA, &GPIO_InitStructure);                  //根据设定值初始化PA1
	GPIO_ResetBits(GPIOA,GPIO_Pin_1);						//PA1 下拉电位
...
{	 
	...
	GPIO_InitStructure.GPIO_Pin  = GPIO_Pin_2;    		    //PB.0 清除之前设置  
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IPU; 		    //PB.0 上拉输入
	GPIO_Init(GPIOA, &GPIO_InitStructure);                  //根据设定值初始化PB.0
	GPIO_ResetBits(GPIOA,GPIO_Pin_2);						//PB.0 下拉电位
...
void TIM5_IRQHandler(void)
{
	...
	if(TIM_GetITStatus(TIM5, TIM_IT_CC3) != RESET)//通道3发生捕获事件
	{
		...
		{ 			
			...
			TIM5CH3_CAPTURE_VAL=TIM_GetCapture3(TIM3);
