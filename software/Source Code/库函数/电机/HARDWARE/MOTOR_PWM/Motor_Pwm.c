...
/**
  ...
  * 接着调用初始化函数Wheel_Init();
  * 通过MotorRight(TIM4,u16 speed);与 MotorLeft1(TIM4,u16 speed);调节速度
  */
...
void Wheel_Init(void)
{
    ...
    TIM_TimeBaseStructure.TIM_Period = ARR_PWM; //设置在下一个更新事件装入活动的自动重装载寄存器周期的值    50HZ  
    TIM_TimeBaseStructure.TIM_Prescaler =PSC_PWM; //设置用来作为TIMx时钟频率除数的预分频值  不分频  
